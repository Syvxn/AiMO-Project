...
func check_answer(show_color=true) -> bool:
	...
	if item_list.get_item_text(selected_answer_index).begins_with(correct_key):
		if show_color:
			item_list.set_item_custom_bg_color(selected_answer_index, Color("42a964ff"))
		return true
	else:
		if show_color:
			item_list.set_item_custom_bg_color(selected_answer_index, Color("cc2823ff"))
		return false
...
func show_correct_answer():
	for i in range(item_list.item_count):
		if item_list.get_item_text(i).begins_with(correct_key):
			item_list.set_item_custom_bg_color(i, Color("42a964ff"))
